...
Este hotfix fez parte da cadeia executada durante a Fase 4.1, mas a fase inteira foi posteriormente revertida. O arquivo é mantido apenas para rastreabilidade histórica.
...
## Relação com o rollback

- A execução completa da Fase 4.1 foi revertida em `d7a839b` no módulo e em `f670542` no root.
- O bug do hook `post-commit` foi corrigido separadamente em `scripts/sync/install-superproject-sync.ps1` e `.git/hooks/post-commit`.

---

*Rollback histórico preservado via `git revert`; referência pré-fase: `73b3800`*
