...
import { Database, ExternalLink, RefreshCw, ShieldCheck, Trash2, X } from 'lucide-react';
import type React from 'react';
import { useEffect, useState } from 'react';
import { autoDocScheduler, type DocTarget } from '../../services/autoDocScheduler';
...
const AutoDocModal: React.FC<AutoDocModalProps> = ({ isOpen, onClose }) => {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: loadTargets é estável (função local sem deps)
  useEffect(() => {
    if (isOpen) {
      loadTargets();
    }
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: loadTargets é estável (função local sem deps)
  useEffect(() => {
    const handleSyncComplete = (_targetId: string, _success: boolean) => {
      loadTargets();
    };
  ...
  const toggleAutoSync = async (id: string) => {
    ...
    if (!target) return;
    await autoDocScheduler.updateTarget(id, { autoSync: !target.autoSync });
    await loadTargets();
  };

  const handleRemove = async (id: string) => {
    await autoDocScheduler.removeTarget(id);
    await loadTargets();
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-3 py-2 space-y-3 max-h-[60vh]">
          ...
          {isLoading ? (
            ...
            <div className="space-y-2">
              {targets.map((target) => (
                <div key={target.id} className="space-y-1 group">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 overflow-hidden">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <h3 className="text-[10px] font-bold text-glass uppercase tracking-wider truncate">
                            {target.name}
                          </h3>
                          <a
                            href={target.url}
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        ...
                        className={`text-[8px] font-bold uppercase tracking-widest px-1.5 py-0.5 transition-all ${
                          target.status === 'syncing'
                            ? 'text-accent-primary'
                      ...
                      <button
                        ...
                        disabled={target.status === 'syncing'}
                        className="p-0.5 text-glass-muted hover:text-red-400 transition-colors disabled:opacity-30"
                        title="Remover target"
                      >
                        <Trash2 size={10} />
                      </button>
                    </div>
                  ...
                  <div className="flex items-center justify-between pl-5">
                    <label className="flex items-center gap-1.5 cursor-pointer group/check">
                      <div
                        className={`w-2 h-2 border border-glass-muted/50 transition-all ${target.autoSync ? 'bg-accent-primary border-accent-primary' : 'group-hover/check:border-glass-muted'}`}
                      >
                        {target.autoSync && <div className="w-full h-full bg-accent-primary" />}
                      </div>
                      <span className="text-[8px] text-glass-muted uppercase tracking-widest opacity-60 group-hover/check:opacity-100 transition-opacity">
                        Auto-Sync
                      </span>
                      <input
                        type="checkbox"
                    ...
                    {target.lastSynced && (
                      <span className="text-[8px] text-glass-muted opacity-40 font-mono">
                        {new Date(target.lastSynced).toLocaleTimeString([], {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </span>
        ...
        {/* Footer - Compact */}
        <div className="px-3 py-2 glass-header flex items-center justify-between shrink-0">
          <span className="text-[8px] font-bold uppercase tracking-widest text-glass-muted opacity-50">
            Rabelus Lab — Ecosystem
          </span>
        </div>
      </div>
    </div>
