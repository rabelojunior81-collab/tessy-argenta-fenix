...
export const ProjectDocModal: React.FC<ProjectDocModalProps> = ({ isOpen, onClose }) => {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: função local estável sem deps
  useEffect(() => {
    if (isOpen) {
      loadProjects();
    }
            ...
            <div className="flex gap-1">
              {(['minimal', 'standard', 'detailed'] as const).map((style) => (
                <button
                  ...
                  className={`flex-1 py-1 text-[9px] font-medium uppercase tracking-wide transition-all border ${
                    readmeStyle === style
                ...
                <div key={item.id} className="space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    {status.status === 'generating' && (
                      <Loader2 size={10} className="text-glass-accent animate-spin" />
                    )}
