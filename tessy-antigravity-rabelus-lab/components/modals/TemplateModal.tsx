import { Bookmark, ChevronRight, Edit3, Hash, Plus, Search, Trash2, X } from 'lucide-react';
      ...
      >
        ...
        <div className="px-2 py-1 glass-header flex items-center justify-between shrink-0">
          <div className="flex items-center gap-1.5">
            ...
            <h2 className="text-[9px] font-bold tracking-widest text-glass uppercase">
              Protocolos
            </h2>
          </div>
          <button
        ...
        <div className="flex-1 flex flex-row overflow-hidden relative z-10 bg-transparent">
          ...
          <div className="w-[140px] flex flex-col border-r border-glass shrink-0 bg-transparent">
            <div className="p-2 border-b border-glass/10 flex items-center gap-1">
              <div className="flex-1 relative">
                <Search
                  className="absolute left-2 top-1/2 -translate-y-1/2 text-glass-muted"
                  size={10}
                />
                <input
                  type="text"
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-1.5 space-y-3">
              {(Object.entries(groupedTemplates) as [string, Template[]][]).map(
                ([category, items]) => (
                  <div key={category} className="space-y-0.5">
                    ...
                    <div className="space-y-0.5">
                      {items.map((t) => (
                        // biome-ignore lint/a11y/noStaticElementInteractions lint/a11y/useKeyWithClickEvents: template list item
                        <div
                          key={t.id}
                          role="presentation"
                          onClick={() => {
                          ...
                          className={`group px-2 py-1.5 transition-all cursor-pointer flex items-center gap-2 border-l-2 ${
                            selectedId === t.id
                        ...
                        >
                          <Hash
                            size={10}
                            className={selectedId === t.id ? 'text-glass-accent' : 'opacity-20'}
                          />
          ...
          <div className="flex-1 flex flex-col overflow-hidden bg-transparent">
            {isFormOpen ? (
              <form
                onSubmit={handleSave}
                className="flex-1 flex flex-col p-4 space-y-3 animate-fade-in h-full"
              >
                <div className="space-y-3">
                  <div className="flex gap-2">
                    <div className="flex-1 space-y-1">
                      <label
                        htmlFor="tm-label"
                        className="text-[8px] font-bold text-glass-muted uppercase tracking-widest"
                      >
                        Protocolo
                      </label>
                      <input
                    ...
                    <div className="w-[100px] space-y-1">
                      ...
                      >
                        <option value="Código" className="bg-bg-primary">
                          CÓDIGO
                        </option>
                        <option value="Escrita" className="bg-bg-primary">
                          ESCRITA
                        </option>
                        <option value="Análise" className="bg-bg-primary">
                          ANÁLISE
                        </option>
                        <option value="Ensino" className="bg-bg-primary">
                          ENSINO
                        </option>
                        <option value="Criativo" className="bg-bg-primary">
                          CRIATIVO
                        </option>
                        <option value="Personalizado" className="bg-bg-primary">
                          PESSOAL
                        </option>
                      </select>
                    </div>
                ...
                <div className="flex gap-2 pt-1 pb-1">
                  <button
                    ...
                    className="flex-1 py-1.5 glass-card border-glass/10 text-glass-muted hover:text-glass font-bold uppercase tracking-widest text-[8px] transition-all active:scale-95"
                  >
                    Cancelar
                  </button>
                  <button
                    ...
                    className="flex-1 py-1.5 bg-glass-accent text-white hover:brightness-110 font-bold uppercase tracking-widest text-[8px] transition-all active:scale-95 border-transparent"
                  >
                    Salvar
                  </button>
                </div>
              ...
              <div className="flex-1 flex flex-col overflow-hidden animate-fade-in h-full">
                    ...
                    {selectedTemplate.isCustom && (
                      <>
                        <button
                          ...
                          className="p-1.5 text-glass-muted hover:text-glass transition-all"
                        >
                          <Edit3 size={12} />
                        </button>
                        <button
                          ...
                          className="p-1.5 text-glass-muted hover:text-red-400 transition-all"
                        >
                          <Trash2 size={12} />
                        </button>
                      </>
                ...
                <div className="flex-1 p-3 overflow-y-auto custom-scrollbar bg-transparent">
                  <pre className="text-[10px] text-glass-secondary font-mono font-normal whitespace-pre-wrap leading-relaxed p-3 glass-card border-glass/10 bg-black/20">
