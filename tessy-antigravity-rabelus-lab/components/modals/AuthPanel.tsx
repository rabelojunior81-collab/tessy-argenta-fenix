 ...
 *
 * NOTA: Vault/criptografia removido por decisão operacional (2026-03-10).
 * Tokens armazenados em plaintext no IndexedDB. Vault será reimplementado depois.
 */

import { AlertCircle, Check, ExternalLink, ShieldCheck, X } from 'lucide-react';
...
const AuthPanel: React.FC<AuthPanelProps> = ({ isOpen, onClose, onProviderUpdate }) => {
  ...
  const [activeProviderStorageMode, setActiveProviderStorageMode] = useState<
    'transparent' | 'user-secret' | 'legacy' | null
  >(null);

  const activeProvider = AUTH_PROVIDERS.find((p) => p.id === activeTab)!;

  // biome-ignore lint/correctness/useExhaustiveDependencies: loadConnectedProviders e loadCurrentToken são estáveis (funções locais sem deps)
  useEffect(() => {
    if (isOpen) {
      setIsClosing(false);
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: loadCurrentToken é estável (função local sem deps)
  useEffect(() => {
    loadCurrentToken();
    setSaveStatus('idle');
    setIsEditing(false);
  ...
  const loadCurrentToken = async () => {
    const storageMode = await getProviderStorageMode(activeTab);
    setActiveProviderStorageMode(storageMode);
  ...
  const handleClear = async () => {
    ...
    setTokenInput('');
    setActiveProviderStorageMode(null);
    await loadConnectedProviders();
    onProviderUpdate?.();
  };
      ...
      >
        ...
        <div className="px-3 py-2 glass-header border-b border-glass-border flex items-center justify-between shrink-0">
          <div className="flex items-center gap-1.5 overflow-hidden">
            ...
            <h2 className="text-[10px] font-bold uppercase tracking-widest text-glass opacity-90">
          ...
          <button
            type="button"
            onClick={handleClose}
            className="p-0.5 text-glass-muted hover:text-glass transition-all active:scale-95"
          >
            <X size={14} />
          </button>
        </div>
        ...
        <form onSubmit={handleSave} className="p-3 space-y-3">
          <div className="space-y-1">
            <div className="flex items-center justify-between px-1">
              <span className="text-[9px] font-bold text-glass-muted uppercase tracking-widest">
                Token de Acesso
              </span>
              <a
            ...
            <div className="relative">
              <input
                ...
                style={{
                  borderColor:
                    tokenInput && !tokenInput.includes('•')
                      ? activeProvider.validator(tokenInput)
                        ? '#22c55e'
                        : '#ef4444'
                      : undefined,
                }}
            ...
            {activeProviderStorageMode && (
              <p className="text-[8px] text-center uppercase tracking-widest text-glass-muted/60">
                Armazenado: {activeProviderStorageMode === 'legacy' ? 'formato legado' : 'local'}
              </p>
            )}
          </div>
        ...
        <div className="px-3 py-2 border-t border-glass-border bg-black/20">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              <ShieldCheck size={9} className="text-glass-muted/50" />
              <span className="text-[8px] font-bold text-glass-muted/50 uppercase tracking-widest">
                Local
              </span>
            </div>
