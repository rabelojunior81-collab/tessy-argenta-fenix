...
export const WALLPAPERS = [
  ...
  },
  {
    id: 'northern-lights',
    name: 'Aurora',
    url: 'https://images.unsplash.com/photo-1531366936337-7c912a4589a7?q=80&w=2940&auto=format&fit=crop',
  },
  {
    id: 'mountain',
    name: 'Mountain',
  ...
  },
  {
    id: 'ocean',
    name: 'Ocean',
    url: 'https://images.unsplash.com/photo-1505118380757-91f5f5632de0?q=80&w=2826&auto=format&fit=crop',
  },
  {
    id: 'forest',
    name: 'Forest',
    url: 'https://images.unsplash.com/photo-1448375240586-882707db888b?q=80&w=2940&auto=format&fit=crop',
  },
  {
    id: 'city',
    name: 'City Night',
...
const CUSTOM_WALLPAPERS_KEY = 'tessy-custom-wallpapers';

const getCustomWallpaperSettingValue = (id: string) => `custom:${id}`;

const resolveWallpaperUrl = (wallpaper: string, customWallpapers: CustomWallpaper[]): string => {
  if (wallpaper.startsWith('custom:')) {
    const wallpaperId = wallpaper.replace('custom:', '');
    return customWallpapers.find((wp) => wp.id === wallpaperId)?.dataUrl || '';
  }
  return wallpaper;
};
...
export const VisualProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const currentWallpaperUrl =
    resolveWallpaperUrl(settings.wallpaper, customWallpapers) || DEFAULT_SETTINGS.wallpaper;
  ...
  useEffect(() => {
    ...
    // 4. Wallpaper fallback token for CSS-only consumers
  ...
  }, [settings, resolvedTheme, currentWallpaperUrl]);

  // Load custom wallpapers from IndexedDB on mount
  useEffect(() => {
    db.settings.get(CUSTOM_WALLPAPERS_KEY).then((stored) => {
      const wallpapers = stored?.value || [];
      setCustomWallpapers(wallpapers);

      setSettings((prev) => {
        if (prev.wallpaper.startsWith('data:')) {
          const matchingWallpaper = wallpapers.find(
            (wp: CustomWallpaper) => wp.dataUrl === prev.wallpaper
          );
          if (matchingWallpaper) {
            return { ...prev, wallpaper: getCustomWallpaperSettingValue(matchingWallpaper.id) };
          }
          return { ...prev, wallpaper: WALLPAPERS[0].url };
        }
        return prev;
      });
    });
  ...
  const addCustomWallpaper = useCallback(async (file: File) => {
    ...
    setCustomWallpapers((prev) => {
      ...
      db.settings.put({ key: CUSTOM_WALLPAPERS_KEY, value: updated });
      return updated;
    });
    // Activate it immediately
    setSettings((prev) => ({ ...prev, wallpaper: getCustomWallpaperSettingValue(newWp.id) }));
  }, []);
  ...
  const removeCustomWallpaper = useCallback(async (id: string) => {
    setCustomWallpapers((prev) => {
      ...
      db.settings.put({ key: CUSTOM_WALLPAPERS_KEY, value: updated });
      // If active wallpaper was deleted, reset to first preset
      setSettings((s) => {
        if (wp && s.wallpaper === getCustomWallpaperSettingValue(wp.id)) {
          return { ...s, wallpaper: WALLPAPERS[0].url };
        }
        return s;
      });
      return updated;
