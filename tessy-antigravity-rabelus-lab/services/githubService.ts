...
import { getGitHubAccessToken, setGitHubAccessToken } from './authProviders';
...
const resolveToken = async (token?: string | null): Promise<string> => {
  ...
  const sessionToken = await getGitHubAccessToken();
  if (sessionToken) return sessionToken;
  throw new GitHubError({
    code: 'INVALID_TOKEN',
    message: 'Não há token GitHub ativo.',
    status: 401,
    suggestedAction: 'Conecte o GitHub antes de tentar esta operação.',
  });
};
...
export const fetchRepo = async (token: string, repoPath: string): Promise<GitHubRepo> => {
  const response = await fetch(`${GITHUB_API_BASE}/repos/${repoPath}`, {
    headers: createGitHubHeaders(await resolveToken(token)),
  });
...
export const searchCode = async (
  ...
  const response = await fetch(
    `${GITHUB_API_BASE}/search/code?q=${encodeURIComponent(query)}+repo:${repoPath}`,
    {
      headers: createGitHubHeaders(await resolveToken(token)),
    }
  );
...
export const fetchRepositoryStructure = async (
  token: string,
  repoPath: string,
  maxDepth = 2
): Promise<any> => {
...
export const performCreateBranch = async (
  token: string,
  repoPath: string,
  branchName: string,
  fromBranch: string
  ...
  const refResponse = await fetch(
    `${GITHUB_API_BASE}/repos/${repoPath}/git/refs/heads/${fromBranch}`,
    {
      headers: createGitHubHeaders(await resolveToken(token)),
    }
  );
...
export const performCommitChanges = async (
  token: string,
  repoPath: string,
  ...
  const commitResponse = await fetch(
    `${GITHUB_API_BASE}/repos/${repoPath}/git/commits/${lastCommitSha}`,
    {
      headers: createGitHubHeaders(await resolveToken(token)),
    }
  );
  ...
  const updateRefResponse = await fetch(
    `${GITHUB_API_BASE}/repos/${repoPath}/git/refs/heads/${branch}`,
    {
      method: 'PATCH',
      headers: createGitHubHeaders(await resolveToken(token)),
      body: JSON.stringify({
...
export const performCreatePullRequest = async (
  token: string,
  repoPath: string,
  title: string,
  body: string,
...
export const createPullRequest = async (
  ...
  return {
    success: true,
    status: 'pending_approval',
    message: 'Pull Request aguardando aprovação.',
  };
};
