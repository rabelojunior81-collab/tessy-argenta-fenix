...
export function createFSAdapter(rootHandle: FileSystemDirectoryHandle): FSAAdapter {
  ...
  async function navigateToPath(
    ...
    // Normalize path: remove leading slashes, trailing slashes, and current-dir markers
    const normalizedPath = pathStr.replace(/^\/+/, '').replace(/\/+$/, '');
    if (!normalizedPath || normalizedPath === '.') return rootHandle;

    // Filter out empty segments and '.' (current dir) — isomorphic-git uses dir='.' which
    // produces paths like './.git', './src/file.ts', etc.
