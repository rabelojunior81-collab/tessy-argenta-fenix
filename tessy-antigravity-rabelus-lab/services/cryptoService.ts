...
let currentSaltStorageKey: string | null = null;
...
export async function initializeSecurity(
  ...
  saltStorageKey: string = 'security-salt'
): Promise<boolean> {
  try {
    currentSaltStorageKey = saltStorageKey;
    // 1. Get or Generate Master Salt
    const saltStored = await db.settings.get(saltStorageKey);
...
export function resetSecurity(): void {
  cachedKey = null;
  masterSalt = null;
  currentSaltStorageKey = null;
}

export function getCurrentSaltStorageKey(): string | null {
  return currentSaltStorageKey;
}
...
export async function decryptData(encryptedData: EncryptedData): Promise<string> {
  ...
  // Current policy: fail hard on mismatch to avoid silently returning corrupted secrets.
  try {
