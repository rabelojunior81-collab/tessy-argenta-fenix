...
import { type FileEntry, listDirectory, openDirectory } from './fileSystemService';
import {
  createFSAdapter,
  requestPermission,
...
export async function updateWorkspaceBrokerBinding(
  workspaceId: string,
  payload: Partial<
    Pick<
      Workspace,
      'brokerWorkspaceId' | 'brokerStatus' | 'brokerValidatedAt' | 'brokerRegisteredAt'
    >
  >
): Promise<void> {
  const workspace = await db.workspaces.get(workspaceId);
