...
import type { FSAAdapter } from './fsaAdapter';
 ...
 */
export async function stageFiles(fs: FSAAdapter, dir: string, filepaths: string[]): Promise<void> {
  for (const filepath of filepaths) {
    const matrix = await git.statusMatrix({ fs, dir, filepaths: [filepath] });
    const [, head, workdir] = matrix[0] || [filepath, 0, 0];
 ...
 */
export async function status(fs: FSAAdapter, dir: string): Promise<GitStatusResult[]> {
 ...
 */
export async function currentBranch(fs: FSAAdapter, dir: string): Promise<string | null> {
  try {
 ...
 * List all branches
 */
export async function listBranches(fs: FSAAdapter, dir: string): Promise<string[]> {
 ...
 * Checkout a branch
 */
export async function checkout(fs: FSAAdapter, dir: string, ref: string): Promise<void> {
 ...
 */
export async function isGitRepo(fs: FSAAdapter, dir: string): Promise<boolean> {
  try {
