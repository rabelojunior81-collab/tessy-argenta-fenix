...
- **Global scope**: `E:/tessy-argenta-fenix/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/tessy-argenta-fenix/.claude/notes/`)
...
2. Glob `E:/tessy-argenta-fenix/.claude/notes/*.md` (if directory exists) — global notes
...
Global (E:/tessy-argenta-fenix/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/tessy-argenta-fenix/.claude/notes/` — works in any directory
