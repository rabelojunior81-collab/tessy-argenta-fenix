...
@E:/tessy-argenta-fenix/.codex/get-shit-done/references/ui-brand.md
@E:/tessy-argenta-fenix/.codex/get-shit-done/references/revision-loop.md
@E:/tessy-argenta-fenix/.codex/get-shit-done/references/gate-prompts.md
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@E:/tessy-argenta-fenix/.codex/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
Extract from {{GSD_ARGS}}: phase number (integer or decimal like `2.1`), flags (`--research`, `--skip-research`, `--gaps`, `--skip-verify`, `--skip-ui`, `--prd <filepath>`, `--reviews`, `--text`, `--bounce`, `--skip-bounce`).
...
**If found:**
1. Read template: `E:/tessy-argenta-fenix/.codex/get-shit-done/templates/VALIDATION.md`
...
SPIKE_FINDINGS_PATH=$(ls ./.codex/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1)
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @E:/tessy-argenta-fenix/.codex/get-shit-done/references/tdd.md to all eligible tasks:
