...
<required_reading>
@E:/tessy-argenta-fenix/.codex/get-shit-done/references/domain-probes.md
@E:/tessy-argenta-fenix/.codex/get-shit-done/references/gate-prompts.md
@E:/tessy-argenta-fenix/.codex/get-shit-done/references/universal-anti-patterns.md
</required_reading>
...
- Read and execute @E:/tessy-argenta-fenix/.codex/get-shit-done/workflows/discuss-phase-power.md end-to-end
...
SPIKE_FINDINGS=$(ls ./.codex/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1)
SKETCH_FINDINGS=$(ls ./.codex/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1)
...
```bash
PROFILE_CONTENT=$(cat "E:/tessy-argenta-fenix/.codex/get-shit-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @E:/tessy-argenta-fenix/.codex/agents/gsd-advisor-researcher.md for your role and instructions.
...
1. Parse `--auto` and `--chain` flags from {{GSD_ARGS}}. Note: --all is NOT an auto-advance trigger — it only affects area selection. A session with `--all` but without `--auto` or `--chain` returns to manual next-steps after discussion completes.
...
**Full step-by-step instructions:** @E:/tessy-argenta-fenix/.codex/get-shit-done/workflows/discuss-phase-power.md
