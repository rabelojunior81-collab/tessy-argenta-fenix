...
- **Global scope**: `E:/tessy-argenta-fenix/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/tessy-argenta-fenix/.gemini/notes/`)
...
2. Glob `E:/tessy-argenta-fenix/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (E:/tessy-argenta-fenix/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/tessy-argenta-fenix/.gemini/notes/` — works in any directory
