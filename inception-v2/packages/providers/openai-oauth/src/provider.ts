...
export class OpenAIOAuthProvider implements IProvider {
  readonly id = ProviderId.OpenAIOAuth;
  ...
  private assertClient(): OpenAI {
    if (!this.client) {
      throw new ProviderError(
        'OpenAI OAuth provider is not initialized. Call initialize() first.',
  ...
  async initialize(config: ProviderConfig): Promise<void> {
    ...
    // config.apiKey holds the OAuth access_token — passed as Bearer token via the SDK's apiKey field
    try {
      ...
      throw new ProviderError(
        `Failed to initialize OpenAI OAuth client: ${err instanceof Error ? err.message : String(err)}`,
        ProviderId.OpenAIOAuth,
        { cause: String(err) }
      );
    }
  ...
  async generate(request: GenerateRequest): Promise<GenerateResponse> {
    ...
    try {
      ...
      if (!choice) {
        throw new ProviderError('OpenAI OAuth returned no choices', ProviderId.OpenAIOAuth);
      }
      ...
      throw new ProviderError(
        `OpenAI OAuth generate failed: ${err instanceof Error ? err.message : String(err)}`,
        ProviderId.OpenAIOAuth,
        { cause: String(err) }
      );
    }
  ...
  async *generateStream(request: GenerateRequest): AsyncIterable<GenerateChunk> {
    ...
    try {
      ...
      throw new ProviderError(
        `OpenAI OAuth stream setup failed: ${err instanceof Error ? err.message : String(err)}`,
        ProviderId.OpenAIOAuth,
        { cause: String(err) }
      );
    }
    ...
    try {
      ...
      throw new ProviderError(
        `OpenAI OAuth stream failed: ${err instanceof Error ? err.message : String(err)}`,
        ProviderId.OpenAIOAuth,
        { cause: String(err) }
      );
    }
  ...
  async embed(request: EmbeddingRequest): Promise<EmbeddingResponse> {
    ...
    try {
      ...
      throw new ProviderError(
        `OpenAI OAuth embed failed: ${err instanceof Error ? err.message : String(err)}`,
        ProviderId.OpenAIOAuth,
        { cause: String(err) }
      );
    }
