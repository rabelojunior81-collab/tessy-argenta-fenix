// ============================================================================
// Z.ai Provider Implementation — OpenAI-compatible
// ============================================================================
...
function resolveBaseUrl(cfg: ZAiConfig): string {
  if (cfg.baseUrl) return cfg.baseUrl;
  if (cfg.region) return `https://api.z.ai/${cfg.region}/v1`;
...
export class ZAiProvider implements IProvider {
  ...
  private assertClient(): OpenAI {
    if (!this.client) {
      throw new ProviderError(
        'Z.ai provider is not initialized. Call initialize() first.',
        ProviderId.ZAi
      );
    }
    return this.client;
  ...
  async initialize(config: ProviderConfig): Promise<void> {
    ...
    try {
      ...
      throw new ProviderError(
        `Failed to initialize Z.ai client: ${err instanceof Error ? err.message : String(err)}`,
        ProviderId.ZAi,
        { cause: String(err) }
      );
    }
  ...
  async generate(request: GenerateRequest): Promise<GenerateResponse> {
    ...
    try {
      ...
      if (!choice) {
        throw new ProviderError('Z.ai returned no choices', ProviderId.ZAi);
      }
      ...
      throw new ProviderError(
        `Z.ai generate failed: ${err instanceof Error ? err.message : String(err)}`,
        ProviderId.ZAi,
        { cause: String(err) }
      );
    }
  ...
  async *generateStream(request: GenerateRequest): AsyncIterable<GenerateChunk> {
    ...
    try {
      ...
      throw new ProviderError(
        `Z.ai stream setup failed: ${err instanceof Error ? err.message : String(err)}`,
        ProviderId.ZAi,
        { cause: String(err) }
      );
    }
    ...
    try {
      ...
      throw new ProviderError(
        `Z.ai stream failed: ${err instanceof Error ? err.message : String(err)}`,
        ProviderId.ZAi,
        { cause: String(err) }
      );
    }
  ...
  async embed(_request: EmbeddingRequest): Promise<EmbeddingResponse> {
    throw new ProviderError('Z.ai does not support embeddings', ProviderId.ZAi);
  }
