...
export class OpenAIProvider implements IProvider {
  ...
  async initialize(config: ProviderConfig): Promise<void> {
    ...
    try {
      ...
      throw new ProviderError(
        `Failed to initialize OpenAI client: ${err instanceof Error ? err.message : String(err)}`,
        ProviderId.OpenAI,
        { cause: String(err) }
      );
    }
  ...
  async generate(request: GenerateRequest): Promise<GenerateResponse> {
    ...
    try {
      ...
      if (!choice) {
        throw new ProviderError('OpenAI returned no choices', ProviderId.OpenAI);
      }
  ...
  async *generateStream(request: GenerateRequest): AsyncIterable<GenerateChunk> {
    ...
    try {
      ...
      throw new ProviderError(
        `OpenAI stream setup failed: ${err instanceof Error ? err.message : String(err)}`,
        ProviderId.OpenAI,
        { cause: String(err) }
      );
    }
